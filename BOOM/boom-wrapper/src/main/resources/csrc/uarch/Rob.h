...
static constexpr const char *getUopName(const uint8_t uopc) {
  switch (uopc) {
  case 0:
    return "uopNOP";
  case 1:
...
static constexpr const uint64_t getFetchQueuePc(const TOP *top,
...
struct RobState {
  uint64_t head = 0, head_lsb = 0, head_idx = 0;
  uint64_t tail = 0, tail_lsb = 0, tail_idx = 0;

  /// Two-issue pipeline => two RoBs.
  RobEntry entry[ROB_SIZE][2];
...
#undef READ_ROB_ENTRY
  ...
  RobEntry getTaintingInstruction(const BusyMask &previouslyBusy,
    ...
    if (useTaintedRegister.size() == 0) {
      TRACE("[ERROR] Did not find the tainting instruction %lx\n", addr);
      return {0};
    }
