...
static Bim bim;
static LoopPredictor loop;

void cycleCallback(TEST_HARNESS *top, uint64_t cycles) {
    if (std::getenv("PRINT_CYCLES")) {
      std::cerr << "================ CYCLE " << cycles << "==============\n\n";
    }

    if (std::getenv("PRINT_ROB")) {
      RobState state;
      state.readFrom(top->rootp);
      std::cerr << "\"robState\": " << state.to_string() << ",\n";
    }

    if (std::getenv("PRINT_REGFILE")) {
      std::cerr << "\"REGFILE\": {\n" << regfile::printRegFile(top->rootp) << "\n}\n";
    }

    if (std::getenv("PRINT_BIM")) {
      std::string bimDiff = bim.printDiff(top->rootp);
      if (bimDiff != "")
        std::cerr << "\"BIM DIFF\": {\n" << bimDiff << "\n}\n";
    }

    if (std::getenv("PRINT_LOOP")) {
...
done_processing:
  ...
  do {
    ...
    cycleCallback(tile, trace_count);
  }
